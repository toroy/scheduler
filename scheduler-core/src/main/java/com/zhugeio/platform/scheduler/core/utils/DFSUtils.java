package com.zhugeio.platform.scheduler.core.utils;

import com.zhugeio.platform.scheduler.core.enums.ResStorageFSType;
                ...
                if (configuration == null) {
                    try {
                        ...
                        switch (resStorageFSType){
                            case S3:
                                configuration.set(FS_DEFAULT_FS, PropertyUtils.getString(FS_DEFAULT_FS));
                                configuration.set(FS_S3A_ENDPOINT, PropertyUtils.getString(FS_S3A_ENDPOINT));
                                ...
                                if (StringUtils.isNotEmpty(PropertyUtils.getString(FS_S3A_ACCESS_KEY))) {
                                    configuration.set(FS_S3A_ACCESS_KEY, PropertyUtils.getString(FS_S3A_ACCESS_KEY));
                                    configuration.set(FS_S3A_SECRET_KEY, PropertyUtils.getString(FS_S3A_SECRET_KEY));
                                }else {
                                    // 兼容白名单认证方式
                            ...
                            case OSS:
                                ...
                                configuration.set(FS_DEFAULT_FS, PropertyUtils.getString(FS_DEFAULT_FS));
                                configuration.set(FS_OSS_ENDPOINT, PropertyUtils.getString(FS_OSS_ENDPOINT));
                                configuration.set(FS_OSS_ACCESS_KEY_ID, PropertyUtils.getString(FS_OSS_ACCESS_KEY_ID));
                                configuration.set(FS_OSS_ACCESS_KEY_SECRET, PropertyUtils.getString(FS_OSS_ACCESS_KEY_SECRET));

                                if (StringUtils.isNotBlank(PropertyUtils.getString(FS_OSS_SECURITY_TOKEN))){
                                    configuration.set(FS_OSS_SECURITY_TOKEN, PropertyUtils.getString(FS_OSS_SECURITY_TOKEN));
                                }
                                if (StringUtils.isNotBlank(FS_OSS_MULTIPART_DOWNLOAD_SIZE)) {
                                    configuration.set(FS_OSS_MULTIPART_DOWNLOAD_SIZE, PropertyUtils.getString(FS_OSS_MULTIPART_DOWNLOAD_SIZE));
                                }
                                fs = FileSystem.get(configuration);
                            ...
                            case COS:
                                ...
                                configuration.set("fs.cosn.userinfo.secretId", PropertyUtils.getString(FS_COS_ACCESS_KEY_ID));
                                configuration.set("fs.cosn.userinfo.secretKey", PropertyUtils.getString(FS_COS_ACCESS_KEY_SECRET));
...
//                                configuration.set("fs.cosn.retry.interval.seconds", "3");
                                fs = FileSystem.get(URI.create(PropertyUtils.getString(FS_DEFAULT_FS)),configuration);
                                break;
                            case HDFS:
                                default:
                                    ...
                                    if(defaultFS.startsWith("file")){
                                        String defaultFSProp = PropertyUtils.getString(FS_DEFAULT_FS);
    ...
    public static String getDfsDataBasePath() {
        String basePath = PropertyUtils.getString(DATA_STORAGE_DFS_BASE_PATH);
        if ("/".equals(basePath)) {
...
//    }
    ...
    public static String getDfsFileFullPath(String userResourceDir, String filename){
        String baseScheme = PropertyUtils.getString(SCHEDULER_SCRIPT_BASE_SCHEME,DEFAULT_SCHEDULER_SCRIPT_BASE_SCHEME);
